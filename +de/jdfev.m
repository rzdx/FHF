function [minv,minpara,gminv,gmeanv,suCR,suF] = jdfev(Gmax,NP,d,evfnnm,evn,p,c,ctrtag)
uCR=0.5;
uF=0.5;
A=[];
...
for G=1:Gmax
    ...
    if (ismember(G,round(Gmax*((1:4)/4)))||G==1)&&ctrtag==1
        ctrplot(X,evfnnm,evn,-100,-100,100,100,G);
    end
end
...
function ctrplot(X,evfnnm,evn,xmin,ymin,xmax,ymax,G)
% ptfnm='.png';
ptfnm='.fig';
fg=figure;
...
pause(0.01);
saveas(fg,[fio.addslash(1,'result','contour'),'contour_f',num2str(evn),'_g',num2str(G),ptfnm]);
hold off
close all;
end
